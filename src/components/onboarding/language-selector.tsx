...
const LanguageSelector = forwardRef<BottomSheetMethods, ILanguageSelector>(
        ...
        <BottomSheetView>
          ...
          <ScrollView>
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableWithoutFeedback
                  onPress={() => handleSelectLanguages(item)}
                  style={{ marginBottom: theme.space[12] }}
                >
                  <View
                    style={{ borderRadius: theme.sizes[12] }}
                    className={`${
                      value?.includes(item) ? "bg-coffee-100" : "bg-grey"
                    }`}
                  >
                    <Flex
                      px={12}
                    ...
                    >
                      ...
                      <Pressable onPress={() => handleSelectLanguages(item)}>
                        {value?.includes(item) ? (
                          <CheckedIcon />
                        ) : (
                          <UncheckedIcon />
